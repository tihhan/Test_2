...
class UserAdapter(
    ...
    private fun saveUser(user: User) {
        val userList = retrieveUserList().toMutableList() // Retrieve the list of users from SharedPreferences
        val existingUser = userList.find { it.name == user.name } // Find the user with the same name if exists
        if (existingUser != null) {
            userList.remove(existingUser) // Remove the existing user from the list if found
        }
        userList.add(user) // Add the updated user to the list

        val editor = getSharedPreferencesEditor() // Get SharedPreferences Editor for saving data
        val json = Gson().toJson(userList) // Convert the user list to JSON format
        editor.putString("user_list", json) // Save the JSON string to SharedPreferences
